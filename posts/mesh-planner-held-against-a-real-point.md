...
origin: A morning in the home lab with a switch console, Central's Commands tool and a point that would not become a point
---
...
Two floors, three access points. An AP-505H and an AP-735 stayed on the wire as portals. An AP-635 was told it was a point, then had its switch port shut with the PoE left on, so it booted with power and no link, which is the only condition under which an AOS 10 AP will take the point role it was given.
...
**Pulling the link is not pulling the power.** On an AOS-CX switch, `shutdown` on the port drops the link and keeps delivering PoE. That is exactly what a mesh test wants, an AP with power and no Ethernet, but only if the AP reboots into that state. `no power-over-ethernet` then `power-over-ethernet` on the port is the remote power button. Central's MultiEdit editor would not save the PoE line for me; the console did.
...
The reason was channels. Overnight, AirMatch had put the 735 on channel 120 and the 505H on 149. A point scans when it boots, picks a parent, and then parks its backhaul radio on that parent's channel. The 635's own monitor table, which lists every BSSID its radio has heard, had no entry at all for the 505H's 5 GHz radios. The 505H was not losing the selection. It was not in the race.

So I pinned the 735 to 149E, the same 80 MHz channel as the 505H. Three and a half minutes later the 635 had reselected: parent 505H, 720 Mb/s down, 400 to 600 up, the 735 demoted to neighbour. If you want two portals to be each other's fallback, they have to be on one channel, and I would not leave that to the channel planner.

{{figure: fig-mesh-reselect.svg | The point could only hear the portal whose channel its radio was parked on. Putting both portals on one channel is what made the second one a candidate.}}
...
| 635 to 505H, rate | MCS 4 | MCS 7 down, MCS 4 to 6 up, at SNR 25 to 30 |

Two budgets right to within the spread of the measurement. One rate wrong by three MCS steps, which on 80 MHz is the difference between about 430 Mb/s and 720. For a tool whose whole output is a capacity ceiling, that is a factor of two error, and it came from two conservative choices stacked on top of each other.
...
While the 635 was on the marginal link, `show ap mesh link` reported the parent at RSSI 46. After it moved to the good link, it reported the new parent at RSSI 46. The 735 at a real SNR of 9 and the 505H at a real SNR of 30 got the same number. On the build I was running, 10.8.1.0, that column does not move, and I have no idea what it is reporting. What I do know is that the rates in the same row told the truth both times, and that nobody should put that RSSI into a link budget.

The honest read is `show ap monitor ap-list`. It lists every BSSID the radio hears with a current and average RSSI in dBm, a current and average SNR, and a `pathloss` column the AP computes itself. That pathloss is the number the planner budgets, measured by the device on the far end of the link, and it agreed with AirMatch's figure to within 2 dB.
...
| Put portals that are meant to back each other up on one channel | A point parks on its parent's channel and cannot see a portal on another one |
