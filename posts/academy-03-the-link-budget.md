...
My derivation put zero in that slot. EIRP = RSSI + FSPL only works if the receiver saw the antenna's peak. It didn't, and sixty degrees off the peak of a 5.5 dBi antenna is usually worth five or six decibels. Put that loss back in and the derived EIRP comes out near 30 dBm, not 25. Which is exactly the number you'd get if 25 were conducted power with the antenna added on top, the reading I'd spent two days arguing against.

So read plainly, the bench leaned mildly toward the answer I didn't want. It settles nothing either way, because I never measured the off-lobe loss or the Sidekick's own antenna, and those two unknowns are bigger than the difference I was trying to see. Two unmeasured shapes, one equation, and an answer that happened to land on the number I was hoping for.
