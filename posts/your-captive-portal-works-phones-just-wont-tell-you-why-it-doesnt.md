...
- **The ClearPass portal FQDN matches the ClearPass cert and resolves in pre auth.** The pre auth role allows DHCP and DNS, carries the captive portal redirect for the client's HTTP and HTTPS probes, and permits 443 to ClearPass and to the captive portal address. Nothing more.
...
- **AOS 10.** On a tunneled SSID the gateway is the authenticator and sends the RADIUS itself, with the cluster VIP as the NAS-IP, so the gateway goes into ClearPass and the APs don't. On a bridged SSID the AP talks to ClearPass itself. Current code uses securelogin.hpe.com as the portal name, and gateways run a demo self signed cert until you install yours.
